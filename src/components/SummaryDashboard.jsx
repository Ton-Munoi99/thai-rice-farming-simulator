...
export default function SummaryDashboard({ simulation }) {
  ...
  const metrics = [
    ...
    [
      t(language, "pestDiseaseRisk"),
      `${model.pestDiseaseRisk}%`,
      model.pestDiseaseRisk <= 25 ? "#2f8f4e" : model.pestDiseaseRisk <= 50 ? "#e0a82e" : "#d2603a",
      METRIC_QUALITY.score,
    ],
    ...
    [
      t(language, "financialRisk"),
      pickLang(language, model.financialRisk.level, model.financialRisk.levelTh),
      model.financialRisk.tone === "good" ? "#2f8f4e" : model.financialRisk.tone === "warning" ? "#e0a82e" : "#d2603a",
        ...
        <header className="flex items-center justify-between rounded-t-xl border-b border-[#d8ddd2] bg-[#2f5d50] px-[26px] py-5 text-white">
          ...
          <div className="text-right">
            ...
            <div className="text-[11px] opacity-90">
              kg/rai · {t(language, "growthScore")} {model.growthScore}/100
            </div>
          </div>
        </header>
          ...
          <div className="grid grid-cols-2 gap-[11px] md:grid-cols-5">
            {metrics.map(([name, value, color, quality]) => (
              <div key={name} className="rounded-lg border border-rice-card bg-white px-[13px] py-3">
                ...
                <div className="mt-1 font-display text-[19px] font-bold" style={{ color }}>
                  {value}
                </div>
              </div>
            ))}
          ...
          <div className="mt-3.5 rounded-lg border border-[#d8ddd2] bg-[#fbfaf6] px-[18px] py-[15px]">
            <div className="mb-2 flex items-baseline justify-between gap-3">
              <div>
                ...
                <div className="text-[10px] text-rice-faint">
                  {simulation.farmSize} {t(language, "rai")}
                </div>
              </div>
          ...
          >
            <div>
              <div className="text-[12px] opacity-90">
                {profitPositive ? t(language, "profit") : t(language, "loss")} {t(language, "perRai")}
              </div>
              ...
              <div className="mt-0.5 text-[10px] opacity-75">
                ...
                {formatNumber(model.straw.revenuePerRai)}/{t(language, "rai")}
              </div>
            </div>
