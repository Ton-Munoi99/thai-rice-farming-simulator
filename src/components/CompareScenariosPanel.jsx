  ...
  return (
    <section className="mt-3.5 rounded-[13px] border border-rice-card bg-white px-[13px] py-3">
      <div className="mb-2 flex items-start justify-between gap-2">
        <div>
          <div className="text-[11px] font-bold text-[#3c473a]">Compare scenarios</div>
          <div className="text-[9.5px] leading-snug text-rice-faint">บันทึก A/B/C แล้วเทียบผลผลิต ต้นทุน กำไร ความเสี่ยง</div>
        </div>
      </div>
      ...
      {savedSlots.length === 0 ? (
        <div className="rounded-[10px] border border-dashed border-[#d7e0cf] bg-[#fbfcf8] px-3 py-3 text-center text-[10px] leading-snug text-rice-faint">
          ตั้งค่าปัจจุบัน แล้วกด Save A/B/C เพื่อเริ่มเทียบ
        </div>
      ) : (
...
function CompareRow({ slot, isBest, onLoad, onClear }) {
      ...
      <div className="grid grid-cols-[42px_1fr_1fr] items-start gap-1.5">
        ...
        <div className="min-w-0">
          <div className="truncate text-[10px] font-bold text-[#3c473a]">
            {slot.varietyName} · {slot.savedAt}
          </div>
          <div className="mt-0.5 text-[9px] leading-snug text-rice-faint">
            {formatNumber(model.estimatedYieldKgPerRai)} kg/rai · Risk {model.riskLevel}
          </div>
          <div className="mt-0.5 text-[8.5px] leading-snug text-[#9aa394]">
            Cost ฿{formatNumber(model.costPerRai)} · Rev ฿{formatNumber(model.revenuePerRai)}
          </div>
        </div>
        <div className="text-right">
          ...
          {isBest ? <div className="text-[8px] font-bold text-rice-green">best</div> : null}
        </div>
      </div>
